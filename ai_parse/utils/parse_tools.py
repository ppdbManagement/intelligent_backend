...
def doc_parse(doc):
    ...
    startParseStatus = DocumentParseStatus.objects.create(
        document=doc, status="doc_parse", start_end_flag="start")
    # 生成一个路径的uuid
    ...
    try:
        ...
        endParseStatus = DocumentParseStatus.objects.create(
            document=doc, status="doc_parse", start_end_flag="end", previous_status=startParseStatus, parse_result=docParseResult)
...
def metadata_extract(doc, prev):
    ...
    startParseStatus = DocumentParseStatus.objects.create(
        document=doc, status="metadata_extract", start_end_flag="start", previous_status=prev_status)
    try:
        # 进行元数据提取的具体实现
        ...
        docParseResult = DocumentParseResult.objects.create(
            result_path=path_uid, literature_info=literature_info)
        endParseStatus = DocumentParseStatus.objects.create(
            document=doc, status="metadata_extract", start_end_flag="end", previous_status=startParseStatus, parse_result=docParseResult)
        doc.current_status = "Stopping"
        doc.save()
...
def table_locate(doc, prev):
    prev_status = DocumentParseStatus.objects.filter(uuid=prev).first()
    startParseStatus = DocumentParseStatus.objects.create(
        document=doc, status="table_locate", start_end_flag="start", previous_status=prev_status)
    try:
        # 先创建一个解析结果的目录
        path_dir, path_uid = make_parse_dir(doc)
        ...
        # 将结果进行融合
        tables_with_related_segments = integrate_related_segments(
            paginated_merged_tables, related_segments)
        with open(os.path.join(path_dir, "tables_with_related_segments.json"), "w", encoding="utf-8") as f:
            json.dump(tables_with_related_segments,
        ...
        docParseResult = DocumentParseResult.objects.create(
            result_path=path_uid, experiment_table_result=experiment_table_result)
        endParseStatus = DocumentParseStatus.objects.create(
            document=doc, status="table_locate", start_end_flag="end", previous_status=startParseStatus, parse_result=docParseResult)
        doc.current_status = "Stopping"
        doc.save()
...
def table_reconstruct(doc, prev):
    prev_status = DocumentParseStatus.objects.filter(uuid=prev).first()
    startParseStatus = DocumentParseStatus.objects.create(
        document=doc, status="table_reconstruct", start_end_flag="start", previous_status=prev_status)
    try:
        reconstruct_table_result = reconstruct_table(prev)
        # 先把结果存储到一个文件里
        path_dir, path_uid = make_parse_dir(doc)
        result_path = os.path.join(path_dir, "reconstruct_table_result.json")
        with open(result_path, "w", encoding="utf-8") as f:
            json.dump(reconstruct_table_result, f,
                      ensure_ascii=False, indent=4)
        # 创建一个DocumentParseResult
        flatParseResult = FlatParseResults.objects.create()
        # 根据结果创建SingleExperimentTableResult
        single_flat_parse_result_list = []
        single_flat_parse_table_header_list = []
        for idx, table in enumerate(reconstruct_table_result):
            # 创建表头对象
            headers = table.get("analysis_table_format_final", [])
            # 如果headers是空，直接跳过，不要这个表了
            if not headers or len(headers) == 0:
                continue
            single_flat_parse_result = SingleFlatParseResult(
                origin_table=SingleExperimentTableResult.objects.filter(
                    uuid=table.get("table_result_uuid")).first(),
                flat_parse_results=flatParseResult,)
            single_flat_parse_result_list.append(single_flat_parse_result)
            for header_idx, header in enumerate(headers):
                single_flat_parse_table_header = SingleFlatParseTableHeader(
                    header_name=header.get("name", ""),
                    header_order=header_idx,
                    single_flat_parse_result=single_flat_parse_result,
                    raw_header_name=header.get("raw_name", ""),
                    description=header.get("brief_description", "")
                )
                single_flat_parse_table_header_list.append(
                    single_flat_parse_table_header)
        # 批量创建 SingleFlatParseResult 和 SingleFlatParseTableHeader
        SingleFlatParseResult.objects.bulk_create(
            single_flat_parse_result_list)
        SingleFlatParseTableHeader.objects.bulk_create(
            single_flat_parse_table_header_list)
        # 创建一个DocumentParseResult
        docParseResult = DocumentParseResult.objects.create(
            result_path=path_uid, flat_parse_result=flatParseResult)
        endParseStatus = DocumentParseStatus.objects.create(
            document=doc, status="table_reconstruct", start_end_flag="end", previous_status=startParseStatus, parse_result=docParseResult)
        doc.current_status = "Stopping"
        doc.save()
        ...
        # 创建一个失败的status
        print(
            f"Error during table reconstruct for document {doc.uuid}: {str(e)}")
        failEndParseStatus = DocumentParseStatus.objects.create(
...
def data_filling(doc, prev):
    prev_status = DocumentParseStatus.objects.filter(uuid=prev).first()
    startParseStatus = DocumentParseStatus.objects.create(
        document=doc, status="data_filling", start_end_flag="start", previous_status=prev_status)
    try:
        filled_tables = fill_table_data(prev)
        # 先把结果存储到一个文件里
        path_dir, path_uid = make_parse_dir(doc)
        result_path = os.path.join(path_dir, "filled_tables.json")
        with open(result_path, "w", encoding="utf-8") as f:
            json.dump(filled_tables, f, ensure_ascii=False, indent=4)
        # 创建一个DocumentParseResult
        docParseResult = DocumentParseResult.objects.create(
            result_path=path_uid)
        endParseStatus = DocumentParseStatus.objects.create(
            document=doc, status="data_filling", start_end_flag="end", previous_status=startParseStatus, parse_result=docParseResult)
        doc.current_status = "Stopping"
        doc.save()
